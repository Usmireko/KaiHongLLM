#!/bin/sh
RUNNER="/data/faultmon/.faultmon.run.sh"
SRC="/data/faultmon/faultmon.sh"
if [ "${FAULTMON_RUNNER:-0}" != "1" ]; then
  if [ "$0" != "$RUNNER" ]; then
    /data/local/tmp/busybox mkdir -p /data/faultmon 2>/dev/null || true
    if /data/local/tmp/busybox cat "$SRC" > "$RUNNER" 2>/dev/null; then
      /data/local/tmp/busybox chmod 755 "$RUNNER" 2>/dev/null || true
      FAULTMON_RUNNER=1 exec /system/bin/sh "$RUNNER" "$@"
    elif /data/local/tmp/busybox cat "$0" > "$RUNNER" 2>/dev/null; then
      /data/local/tmp/busybox chmod 755 "$RUNNER" 2>/dev/null || true
      FAULTMON_RUNNER=1 exec /system/bin/sh "$RUNNER" "$@"
    fi
  fi
...
: "${MEM_PRESSURE_SEC:=10}"
: "${MEM_PRESSURE_FIELD:=available}"   # available|free；默认用 MemAvailable，必要时回退 MemFree
...
evaluate_rules() {
  ...
  # --- mem_pressure：优先 MemAvailable（更符合“真实可用内存”），必要时回退 MemFree ---
  mem_val="$ma"
  mem_field="MemAvailable"

  if [ "${MEM_PRESSURE_FIELD:-available}" = "free" ]; then
    mem_val="$mf"
    mem_field="MemFree"
  else
    # available 模式下：ma 为空/为 0 则回退到 mf
    if [ -z "$mem_val" ] || [ "$mem_val" -le 0 ] 2>/dev/null; then
      mem_val="$mf"
      mem_field="MemFree"
    fi
  fi

  cond_mem=0; [ "$mem_val" -lt "$mem_thr_kb" ] && cond_mem=1
  if rule_consider_ts "mem_pressure" "$cond_mem" "$ts" "$mem_need_ms"; then
    fire_rule_event "mem_pressure" "${mem_field}=${mem_val}KB < ${mem_thr_kb}KB for ${MEM_PRESSURE_SEC}s (mf=${mf}KB, ma=${ma}KB)"
  fi
  ...
  while :; do
    ...
    printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
      ...
      "$disk_r_kBps" "$disk_w_kBps" "$net_rx_kBps" "$net_tx_kBps" >>"$csv"
        ...
        printf '%s\n' "$ts" >"$LAST_TS_MS"
    # rules disabled: collector-only
    # evaluate_rules "$ts" "$mem_free_kb" "$mem_avail_kb" "$load1_x100" "$io_psi_x100"
