...
build_day_list() {
  if [ "$SELFTEST" -eq 1 ] ; then
    ...
    for f in "$EVENT_DIR"/events_*.jsonl; do
      ...
      day="${name#events_}"
      day="${day%.jsonl}"
      if [ -f "$METRICS_DIR/sys_${day}.csv" ]; then
        if [ -z "$latest_both" ] || [ "$day" -gt "$latest_both" ]; then
          latest_both="$day"
        fi
      fi
    done
    if [ -n "$latest_both" ]; then
      day_list="$latest_both"
      SELFTEST_LIMIT=3
      return
    fi
    # 否则退化为原有逻辑：仅按“最近事件日”
...
write_metrics_window() {
  ...
  header_done=0
  rows=0
  MAX_ROWS="${MAX_METRICS_SCAN_LINES:-80000}"  # 可用环境变量覆盖

  # 逐行流式筛选；若第一列（时间戳）单调递增，超出窗口上界后可提前退出
  while IFS= read -r line || [ -n "$line" ]; do
    rows=$((rows + 1))
    ...
    case "$ts_field" in
      ...
      *)
        ...
        # 若 CSV 为时间递增，超过窗口上界可提前退出（加速大文件）
        if [ "$ts_field" -gt "$end" ]; then
          break
        fi
        ;;
    esac
...
write_events_window() {
  ...
  # 环境变量：FAST_EVENTS_TAIL=20000 只从尾部 N 行扫描；否则整文件但有上限
  TAIL_N="${FAST_EVENTS_TAIL:-0}"
  MAX_SCAN="${MAX_EVENTS_SCAN_LINES:-50000}"

  input_file="$src"
  tmp_file=""
  if [ "$TAIL_N" -gt 0 ]; then
    tmp_file="${STATE_DIR:-/data/faultmon/state}/.events_tail_$$.tmp"
    mkdir -p "$(dirname "$tmp_file")" 2>/dev/null || true
    if tail -n "$TAIL_N" "$src" >"$tmp_file" 2>/dev/null; then
      input_file="$tmp_file"
    else
      tmp_file=""
    fi
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    if [ "$TAIL_N" -eq 0 ] && [ "$scanned" -ge "$MAX_SCAN" ] && [ "$matched" -eq 0 ]; then
      break
    fi
  done <"$input_file"
  ...
  if [ "$matched" -eq 0 ]; then
    tail -n 500 "$src" >"$out" 2>/dev/null || : >"$out"
  fi
}
...
write_log_window() {
  ...
  matched=0
  scanned=0
  MAX_SCAN_LINES=${MAX_LOG_SCAN_LINES:-50000}  # 可用环境变量覆盖
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    if [ "$scanned" -ge "$MAX_SCAN_LINES" ] && [ "$matched" -eq 0 ]; then
      break
    fi
  done <"$log_file"
...
process_day() {
  ...
  if [ "${limit:-0}" -gt 0 ]; then
    ...
    if [ "$total_lines" -gt "$limit" ]; then
      tmp_tail="${STATE_DIR:-/data/faultmon/state}/.events_tail_$$.tmp"
      mkdir -p "$(dirname "$tmp_tail")" 2>/dev/null || true
      if tail -n "$limit" "$events_file" >"$tmp_tail" 2>/dev/null; then
        lineno=$((total_lines - limit))
        while IFS= read -r line || [ -n "$line" ]; do
          lineno=$((lineno + 1))
          [ -n "$line" ] || continue
