#!/usr/bin/env python3
"""NET-only contract-aware held-out generation smoke v2.
...
DEFAULT_BASELINE_7D_SUMMARY = (
    ...
    / "net_only_contract_aware_generation_smoke_20260519_20260519_134654/contract_aware_metrics_summary.json"
)
DEFAULT_BASELINE_7D_OUTPUT = (
    DIAGNOSTIC_OUTPUT_ROOT / "net_only_contract_aware_generation_smoke_20260519_20260519_134654"
)
PROMPT_VERSION = "net_only_contract_aware_prompt_v2_20260519"
CONTRACT_VERSION = "net_only_non_action_diagnostic_output_contract_v1"
SCHEMA_VERSION = "net_only_contract_aware_generation_smoke_v2"
SMOKE_NOTE = (
    "Prompt-v2 alignment smoke only: adapter came from max_steps=3 training smoke; "
...
def _contract_task_shape(task: str, contract: Dict[str, Any]) -> str:
    ...
    if task == "evidence_extraction":
        return (
            "Return exactly one JSON object with these four top-level keys in this exact order: "
            '"primary_evidence", "secondary_evidence", "symptom_evidence", "noise_evidence". '
            'The minimal valid empty shape is '
            '{"primary_evidence":[],"secondary_evidence":[],"symptom_evidence":[],"noise_evidence":[]}. '
            'A non-empty item must be an object like {"eid":"e1"} using only contract-allowed evidence item fields. '
            "Every one of the four arrays must be present even when empty. "
            "Never output an array as a string. Never omit a required array. "
            "Never use free-text evidence paragraphs. Never put evidence under an 'evidence' wrapper. "
            "The final character of the answer must be the JSON object closing brace `}`. "
            "Copy candidate_evidence EIDs into the array matching each item's support_role. "
            "Include l2_obs_* EIDs when present; they are ordinary evidence IDs. "
...
def _contract_system_prompt() -> str:
    return (
        ...
        "For evidence_extraction, all four required arrays must always appear, even if some are empty."
    )
...
def _contract_evaluate(
    ...
    return contract_checker._evaluate_dataset(
        "task_7e_contract_aware_v2_predictions",
        joined,
        predictions,
        contract,
        predictions_path,
...
def _load_baseline_7d(path: Path) -> Dict[str, Any]:
    if not path.is_file():
        raise UserError("BASELINE_7D_SUMMARY_NOT_FOUND", f"missing 7D baseline summary: {path}")
    return _load_json(path)


def _comparison_with_7d(summary_7e: Dict[str, Any], baseline_7d: Dict[str, Any]) -> Dict[str, Any]:
    keys = [
        "json_parse_success_rate",
        "target_schema_success_rate",
        "semantic_metric_eligible_rate",
        "evidence_schema_success_rate",
    ...
    comparison = {
        key: {
            "task_7d": baseline_7d.get(key),
            "task_7e_contract_aware_v2": summary_7e.get(key),
        }
        for key in keys
    }
    old_schema = float(baseline_7d.get("target_schema_success_rate") or 0.0)
    new_schema = float(summary_7e.get("target_schema_success_rate") or 0.0)
    old_evidence = float(baseline_7d.get("evidence_schema_success_rate") or 0.0)
    new_evidence = float(summary_7e.get("evidence_schema_success_rate") or 0.0)
    comparison["target_schema_improved_over_7d"] = new_schema > old_schema
    comparison["evidence_schema_improved_over_7d"] = new_evidence > old_evidence
    comparison["prompt_v2_alignment_only_not_model_performance"] = True
    return comparison


def _load_7d_predictions(output_dir: Path) -> List[Dict[str, Any]]:
    path = output_dir / "contract_aware_predictions.jsonl"
    if not path.is_file():
        raise UserError("BASELINE_7D_PREDICTIONS_NOT_FOUND", f"missing 7D predictions: {path}")
    return [json.loads(line) for line in path.open("r", encoding="utf-8") if line.strip()]


def _load_7d_per_sample(output_dir: Path) -> List[Dict[str, Any]]:
    path = output_dir / "per_sample_contract_audit.jsonl"
    if not path.is_file():
        raise UserError("BASELINE_7D_PER_SAMPLE_NOT_FOUND", f"missing 7D per-sample audit: {path}")
    return [json.loads(line) for line in path.open("r", encoding="utf-8") if line.strip()]


def _evidence_failure_analysis(test_rows: List[Dict[str, Any]], baseline_7d_output: Path) -> List[Dict[str, Any]]:
    pred_by_sample = {str(row["sample_id"]): row for row in _load_7d_predictions(baseline_7d_output)}
    test_by_sample = {str(row["sample_id"]): row for row in test_rows}
    failures: List[Dict[str, Any]] = []
    for row in _load_7d_per_sample(baseline_7d_output):
        if row.get("task") != "evidence_extraction" or row.get("target_schema_success"):
            continue
        sample_id = str(row["sample_id"])
        pred = pred_by_sample[sample_id]
        item = test_by_sample[sample_id]
        prediction_text = str(pred.get("prediction_text") or "")
        parsed = cal._safe_json_object(prediction_text)
        cause = "other"
        if parsed is not None and "eid" in parsed and all(field.endswith(":missing") for field in row["schema_errors"]):
            cause = (
                "model produced the intended four evidence arrays but emitted malformed outer JSON closure; "
                "the conservative parser recovered the first nested evidence object, so required top-level arrays appeared missing"
            )
        elif not prediction_text.strip():
            cause = "empty model output"
        elif parsed is None:
            cause = "prediction text is not parseable as a JSON object"
        failures.append(
            {
                "sample_id": sample_id,
                "run_id": item["run_id"],
                "subtype": pred.get("gold_subtype"),
                "task": row["task"],
                "schema_errors": row["schema_errors"],
                "prediction_text": prediction_text,
                "gold_assistant_target": item["gold_text"],
                "failure_category": "malformed_outer_json_closure" if "outer JSON closure" in cause else cause,
                "failure_reason": cause,
                "prompt_v2_fix": (
                    "strengthen evidence_extraction prompt to require all four arrays, forbid string-wrapped arrays, "
                    "and require a complete object ending with `}`"
                ),
            }
        )
    return failures
    ...
    try:
        _json_dump(
            args.output_dir / "generation_manifest.json",
            {
                ...
                "prompt_v2_alignment_only_not_model_performance": True,
                **preflight,
            },
        )
        prompt_text = (
        ...
        (args.output_dir / "generation_prompt_v2.txt").write_text(prompt_text + "\n", encoding="utf-8")
        log.info("starting contract-aware generation")
        predictions = _generate_predictions(args, test_rows, contract, args.output_dir, log)
        predictions_path = args.output_dir / "contract_aware_v2_predictions.jsonl"
        (args.output_dir / "contract_aware_predictions.jsonl").replace(predictions_path)
        summary_7d, per_sample = _contract_evaluate(test_rows, predictions, contract, predictions_path)
        baseline_7d = _load_baseline_7d(args.baseline_7d_summary)
        comparison = _comparison_with_7d(summary_7d, baseline_7d)
        evidence_failures = _evidence_failure_analysis(test_rows, args.baseline_7d_output)
        _json_dump(args.output_dir / "contract_aware_v2_metrics_summary.json", summary_7d)
        _jsonl_write(args.output_dir / "per_sample_contract_audit_v2.jsonl", per_sample)
        _json_dump(args.output_dir / "comparison_with_7d.json", comparison)
        _jsonl_write(args.output_dir / "evidence_failure_analysis.jsonl", evidence_failures)
        summary = {
            "schema_version": SCHEMA_VERSION,
            "result": "PASS",
            ...
            "prompt_version": PROMPT_VERSION,
            "prompt_hash": _sha256_file(args.output_dir / "generation_prompt_v2.txt"),
            "contract_aware_metrics": summary_7d,
            "comparison_with_7d": comparison,
            "evidence_failure_analysis_count": len(evidence_failures),
            "conclusions": {
                "target_schema_improved_over_7d": comparison["target_schema_improved_over_7d"],
                "evidence_schema_improved_over_7d": comparison["evidence_schema_improved_over_7d"],
                "failed_evidence_rows_7d": len(evidence_failures),
                "failed_evidence_rows_7e": sum(
                    1 for row in per_sample if row["task"] == "evidence_extraction" and not row["target_schema_success"]
                ),
