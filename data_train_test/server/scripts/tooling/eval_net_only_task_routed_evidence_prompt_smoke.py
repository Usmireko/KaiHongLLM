#!/usr/bin/env python3
"""NET-only repaired-v2 task-routed evidence prompt generation smoke.
...
cause_vs_symptom samples through the stable prompt-v2-style contract wording,
while evidence_extraction samples use an evidence-specific no-empty prompt-v4.
...
PROMPT_VERSION = "net_only_task_routed_evidence_prompt_v4_20260520"
PROMPT_V2_ROUTE_VERSION = "net_only_contract_aware_prompt_v2_20260519"
PROMPT_V4_ROUTE_VERSION = "net_only_evidence_specific_no_empty_prompt_v4_20260520"
CONTRACT_VERSION = "net_only_non_action_diagnostic_output_contract_v1"
SCHEMA_VERSION = "net_only_task_routed_evidence_prompt_smoke_v1"
...
def _prompt_route(task: str) -> str:
    if task == "diagnosis":
        return "prompt_v2_diagnosis"
    if task == "cause_vs_symptom":
        return "prompt_v2_cause_vs_symptom"
    if task == "evidence_extraction":
        return "prompt_v4_evidence_no_empty"
    raise UserError("UNEXPECTED_TASK", f"unsupported task: {task}")


def _route_prompt_version(task: str) -> str:
    return PROMPT_V4_ROUTE_VERSION if task == "evidence_extraction" else PROMPT_V2_ROUTE_VERSION
...
def _build_prompt_messages(item: Dict[str, Any], contract: Dict[str, Any]) -> Tuple[List[Dict[str, str]], str]:
    task = item["task"]
    route = _prompt_route(task)
    route_version = _route_prompt_version(task)
    contract_text = _contract_task_shape(task, contract)
    evidence_rule = (
        "EVIDENCE NO-EMPTY RULE:\n"
        f"- {_evidence_no_empty_rule_text()}\n\n"
        if task == "evidence_extraction"
        else ""
    )
    user_text = (
        f"PROMPT_VERSION: {route_version}\n"
        f"TASK_ROUTED_PROMPT_VERSION: {PROMPT_VERSION}\n"
        f"PROMPT_ROUTE: {route}\n"
    ...
    messages = [
        {"role": "system", "content": _contract_system_prompt(task)},
...
def _contract_evaluate(
    ...
    return contract_checker._evaluate_dataset(
        "task_9k_task_routed_evidence_prompt_v4_smoke",
        joined,
        predictions,
        contract,
        predictions_path,
...
def _generate_predictions(
    ...
    with torch.inference_mode():
        for idx, item in enumerate(test_rows, 1):
            ...
            prompt_route_rows.append(
                {
                    ...
                    "prompt_route": prompt_route,
                    "route_prompt_version": route_version,
                    "task_routed_prompt_version": PROMPT_VERSION,
                    "prompt_hash": prompt_hash,
                    "evidence_empty_violation": evidence_empty,
                    "adapter_path": str(args.adapter_path),
                    "source_test_jsonl": str(args.test_file),
            ...
            _jsonl_write(output_dir / "task_routed_predictions.jsonl", predictions)
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        prompt_text = (
            "# Task-routed prompt-v4 routes\n\n"
            "## diagnosis route: prompt-v2 stable\n\n"
            + _contract_system_prompt("diagnosis")
            + "\n\n"
            + _contract_task_shape("diagnosis", contract)
            + "\n\n## evidence_extraction route: evidence-specific prompt-v4\n\n"
            + _contract_system_prompt("evidence_extraction")
            + "\n\n"
            + _contract_task_shape("evidence_extraction", contract)
            + "\n\n"
            + _evidence_no_empty_rule_text()
            + "\n\n## cause_vs_symptom route: prompt-v2 stable\n\n"
            + _contract_system_prompt("cause_vs_symptom")
            + "\n\n"
            + _contract_task_shape("cause_vs_symptom", contract)
        )
        ...
        manifest = {
            ...
            "prompt_version": PROMPT_VERSION,
            "task_routed_prompt_hash": _sha256_file(args.output_dir / "generation_prompt_routes.md"),
            "prompt_v2_route_version": PROMPT_V2_ROUTE_VERSION,
            "prompt_v4_route_version": PROMPT_V4_ROUTE_VERSION,
            "contract_version": CONTRACT_VERSION,
            "generation_config": {
                "do_sample": False,
                "max_new_tokens": args.max_new_tokens,
            ...
            "prompt_routes": {
                "diagnosis": "prompt_v2_diagnosis",
                "evidence_extraction": "prompt_v4_evidence_no_empty",
                "cause_vs_symptom": "prompt_v2_cause_vs_symptom",
            },
            "disk_before": disk_before,
            **preflight,
        }
        _json_dump(args.output_dir / "generation_manifest.json", manifest)
        log.info("starting task-routed evidence prompt-v4 generation smoke")
        ...
        predictions_path = args.output_dir / "task_routed_predictions.jsonl"
        ...
        comparison = _compare_with_9g_9j(
            strict_summary,
            relaxed_summary,
            baseline_9g_strict,
            baseline_9g_relaxed,
        ...
        _json_dump(args.output_dir / "task_routed_strict_metrics_summary.json", strict_summary)
        _json_dump(args.output_dir / "task_routed_relaxed_metrics_summary.json", relaxed_summary)
        _jsonl_write(args.output_dir / "task_routed_per_sample_audit.jsonl", per_sample)
        _jsonl_write(args.output_dir / "task_routed_per_sample_relaxed_metrics.jsonl", relaxed_rows)
        _json_dump(args.output_dir / "comparison_with_9g_9j.json", comparison)
        ...
        summary = {
            ...
            "comparison_with_9g_9j": comparison,
        ...
        log.info("task-routed evidence prompt-v4 generation smoke completed")
