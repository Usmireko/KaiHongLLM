...
$items = Invoke-Stage2BoardBridgeOnce -HdcArgs $HdcTargetArgs -Server $Server -ServerPort $ServerPort -RepoDir $RepoDir
if (-not $items -or $items.Count -eq 0) {
  Write-Host "BRIDGE_IDLE"
  exit 0
}
...
foreach ($item in $items) {
  if ($item.ok) {
    ...
    Write-Host ("BRIDGE_FAIL step={0} device={1} file={2} msg={3}" -f $item.step, $item.device, $item.name, $item.msg)
    $bad = $true
  }
}
