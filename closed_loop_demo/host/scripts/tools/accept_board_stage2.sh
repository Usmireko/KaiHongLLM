...
while [ "$i" -le 5 ]; do
  ...
  rc=$?
  echo "$out" | grep -F -e "Text file busy" >/dev/null 2>&1 && a1_ok=0
  [ "$rc" -eq 0 ] || a1_ok=0
...
while [ "$waited" -lt 10 ]; do
  sys_file="$(ls -t /data/faultmon/metrics/sys_*.csv 2>/dev/null | head -n 1)"
  [ -n "$sys_file" ] && break
  sleep 1
  waited=$((waited + 1))
done
if [ -z "$sys_file" ]; then
  ...
  l1="$(tail -n 1 "$sys_file" 2>/dev/null)"
  sleep 6
  l2="$(tail -n 1 "$sys_file" 2>/dev/null)"
...
old_epoch="$(head -n 1 "$STATE_DIR/last_trigger_epoch" 2>/dev/null)"
sleep 2
"$BIN/triggerd.sh" --once --mode cpu --interval 1 --hit_need 1 --cpu_hit_need 1 --threshold_load1_int 999 >/dev/null 2>&1
new_epoch="$(head -n 1 "$STATE_DIR/last_trigger_epoch" 2>/dev/null)"
log_hit="$(tail -n 20 "$LOG" 2>/dev/null | grep -F -e "once_no_trigger" -e "once_trigger_done" | tail -n 1)"
...
while [ "$waited" -lt 20 ]; do
  proc_file="$(ls -t /data/faultmon/procs/procs_*.txt 2>/dev/null | head -n 1)"
  [ -n "$proc_file" ] && break
  sleep 2
  waited=$((waited + 2))
done
...
line="$(tail -n 200 "$LOG" 2>/dev/null | grep -F -e "trigger_bundle run_id=" | tail -n 1)"
if [ -n "$line" ]; then
  run_id="${line#*run_id=}"
  run_id="${run_id%% *}"
fi
ok_line="$(tail -n 200 "$LOG" 2>/dev/null | grep -F -e "OK: uploaded type=bundle" | grep -F -e "${run_id}" | tail -n 1)"
"$BIN/inject_cpu.sh" stop >/dev/null 2>&1 || true
