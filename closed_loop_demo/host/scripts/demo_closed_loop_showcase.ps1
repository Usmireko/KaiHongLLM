...
$serverConfigPath = Join-Path $PSScriptRoot "tools\\server_config.ps1"
...
if (-not $Server -or $Server.Trim() -eq "") { $Server = $Qwen3Cfg.ssh_host }
...
rm -f "$PIDF" "$LOCK" /data/faultmon/state/last_trigger_epoch /data/faultmon/state/last_trigger.json 2>/dev/null

TS=$(date +%Y%m%d_%H%M%S 2>/dev/null)
if [ -z "$TS" ]; then TS=$(date +%Y%m%d_%H%M%S 2>/dev/null); fi
if [ -f "$LOG" ]; then mv "$LOG" "$LOG.$TS" 2>/dev/null; fi

rm -f /data/faultmon/demo_stage2/*__bundle.tar.gz 2>/dev/null
rm -f /data/faultmon/demo_stage2/*__action_result.tar.gz 2>/dev/null
...
function Board-GetLastTriggeredRunId {
  $cmd = @'
LOG=/data/faultmon/demo_stage2/logs/triggerd.log
tail -n 250 "$LOG" 2>/dev/null | grep -F -e "trigger_bundle run_id=" | tail -n 1
'@
  $line = Invoke-Hdc $cmd
...
function Board-HasUploadOk([string]$RunId) {
  ...
  $cmd = @"
LOG=/data/faultmon/demo_stage2/logs/triggerd.log
grep -F -e "OK: uploaded type=bundle" "$LOG" 2>/dev/null | grep -F -e "run=$RunId" | grep -F -e "device=$DeviceId" | tail -n 1
"@
  $line = Invoke-Hdc $cmd
...
function Server-RestartServices {
  return Invoke-Ssh "cd '$RepoDir'; bash closed_loop_demo/server/src/server_B/tcp/demo_services.sh restart; bash closed_loop_demo/server/src/server_B/tcp/demo_services.sh status"
}
...
if ($serverOut -notmatch (":" + [string]$ExpectedIngestPort) -or $serverOut -notmatch (":" + [string]$ExpectedActionsPort)) {
  Write-Host "FAIL: SERVER_SERVICES_NOT_LISTENING"
  Write-Host ("HINT: ssh {0} ""cd {1}; bash closed_loop_demo/server/src/server_B/tcp/demo_services.sh status""" -f $Server, $RepoDir)
  Write-Host ("HINT: expected listen ports {0}/{1}" -f $ExpectedIngestPort, $ExpectedActionsPort)
  exit 1
}
...
if (-not (Board-WaitTriggerdAlive 6)) {
  ...
  Write-Host '  hdc -t $env:HDC_TARGET shell "ls -l /data/faultmon/demo_stage2/pids/triggerd.pid"'
  exit 1
}

Write-Host "READY: run injection in another window (example):"
Write-Host '  hdc -t $env:HDC_TARGET shell "sh /data/faultmon/demo_stage2/bin/inject_cpu.sh start"'
...
if (-not $stage1) {
  ...
  Write-Host 'HINT: hdc -t $env:HDC_TARGET shell "tail -n 200 /data/faultmon/demo_stage2/logs/triggerd.log"'
  exit 1
}
