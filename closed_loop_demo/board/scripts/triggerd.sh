...
latest_metrics_file() {
  ls -t "$METRICS_DIR"/sys_*.csv 2>/dev/null | head -n 1
}

read_faultmon_interval() {
  line="$(grep -m 1 'METRICS_PERIOD_SEC' /data/faultmon/faultmon.sh 2>/dev/null)"
  val="$(printf '%s' "$line" | sed 's/[^0-9]//g')"
...
read_metrics_last() {
  ...
  line="$(tail -n 1 "$metrics_path" 2>/dev/null)"
  [ -z "$line" ] && return 1
...
parse_bundle_output() {
  ...
  upload_rc="$(printf '%s\n' "$out" | grep -F 'upload_rc=' | head -n 1 | cut -d= -f2)"
  case "$upload_rc" in ''|*[!0-9]*) upload_rc="" ;; esac
}

parse_poller_output() {
  out="$1"
  resp_line="$(printf '%s\n' "$out" | grep -F 'resp_header RUN=' | head -n 1)"
  resp_run=""
  resp_len=""
  resp_conclusion=""
  if [ -n "$resp_line" ]; then
  ...
  resp_conc_line="$(printf '%s\n' "$out" | grep 'resp_conclusion=' | head -n 1)"
  if [ -n "$resp_conc_line" ]; then
    resp_conclusion="${resp_conc_line#*resp_conclusion=}"
  fi
  result_bundle="$(printf '%s\n' "$out" | grep -F 'result_bundle=' | head -n 1 | cut -d= -f2)"
...
write_summary() {
  ...
  summary_file="$BASE_DIR/latest_trigger_summary.txt"
  {
    ...
    [ -n "$conclusion" ] && echo "infer_conclusion=$conclusion"
    echo "SUMMARY_END"
  } > "$summary_file"
  ...
  [ -n "$conclusion" ] && log "infer_conclusion=$conclusion"
  log "SUMMARY_END"
}
  ...
  while [ "$attempt" -le 3 ]; do
    ...
    if [ "$resp_run" = "$run_id" ] && [ -n "$resp_len" ] && [ "$resp_len" -gt 0 ] && [ "$poller_rc" -eq 0 ]; then
      ...
      if [ -z "$action_bundle" ]; then
        action_bundle="$(ls -t "$BASE_DIR"/action_result_bundle_${run_id}__*.tar.gz 2>/dev/null | head -n 1)"
      fi
      write_markers "$resp_run" "$poller_rc" "$action_bundle"
      [ -n "$resp_conclusion" ] && echo "$resp_conclusion" > "$BASE_DIR/latest_conclusion.txt" 2>/dev/null || true
      write_summary "$run_id" "$resp_run" "$resp_len" "$poller_rc" "$action_bundle" "$upload_rc" "$WAIT_MISMATCH" "$WAIT_ELAPSED" "$resp_conclusion"
      return 0
    fi
    sleep 1
    attempt=$((attempt + 1))
...
if [ "$1" = "stop" ]; then
  ...
  if [ -z "$pid" ] || ! kill -0 "$pid" 2>/dev/null; then
    ps_out="$(ps w 2>/dev/null || ps 2>/dev/null)"
    echo "$ps_out" | grep -F -e "triggerd.sh" | grep -F -e "--runloop" | grep -F -v -e "grep" | while read -r line; do
      set -- $line
      cand="$1"
...
if [ "$1" = "status" ]; then
  if [ -f "$PIDFILE" ]; then
    ...
    if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
      echo "alive pid=$pid"
      _st_run=""; [ -f "$BASE_DIR/latest_trigger_run_id.txt" ] && _st_run="$(cat "$BASE_DIR/latest_trigger_run_id.txt" 2>/dev/null | head -n 1)"
      [ -n "$_st_run" ] && echo "latest_run_id: $_st_run"
      _st_conc=""; [ -f "$BASE_DIR/latest_conclusion.txt" ] && _st_conc="$(cat "$BASE_DIR/latest_conclusion.txt" 2>/dev/null | head -n 1)"
      [ -n "$_st_conc" ] && echo "infer_conclusion: $_st_conc"
      exit 0
    fi
  fi
  echo "not_running"
...
if [ "$1" = "latest" ]; then
  _lt_run="none"
  _lt_conc="none"
  [ -f "$BASE_DIR/latest_trigger_run_id.txt" ] && _lt_run="$(cat "$BASE_DIR/latest_trigger_run_id.txt" 2>/dev/null | head -n 1)"
  [ -f "$BASE_DIR/latest_conclusion.txt" ] && _lt_conc="$(cat "$BASE_DIR/latest_conclusion.txt" 2>/dev/null | head -n 1)"
  echo "run_id: ${_lt_run:-none}"
  echo "infer_conclusion: ${_lt_conc:-none}"
  exit 0
fi
...
while true; do
  ...
  ts_ms="$(printf '%s' "$ts_ms" | sed 's/[^0-9]//g')"
  load1_x100="$(printf '%s' "$load1_x100" | sed 's/[^0-9]//g')"
  mem_available_kb="$(printf '%s' "$mem_available_kb" | sed 's/[^0-9]//g')"
