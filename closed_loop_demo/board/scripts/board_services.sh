...
ensure_device_id() {
  ...
  dev_id="$(cat "$DEVICE_ID_FILE" 2>/dev/null | head -n 1)"
  if [ -z "$dev_id" ]; then
    echo dev1 > "$DEVICE_ID_FILE"
  fi
}
...
demo_once() {
  ...
  if wait_for_trigger "$mode" "$timeout"; then
    ...
    tail -n 20 "$LOG_DIR/triggerd.log" 2>/dev/null || true
    return 0
  fi
  ...
  tail -n 50 "$LOG_DIR/triggerd.log" 2>/dev/null || true
  tail -n 50 "$LOG_DIR/inject_${mode}.log" 2>/dev/null || true
  return 1
}
