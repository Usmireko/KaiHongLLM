...
EV_FILE="$(ls -t /data/faultmon/events/events_*.jsonl 2>/dev/null | head -n 1)"
...
line="$(tail -n 1 "$SYS_FILE" 2>/dev/null || true)"
ts="${line%%,*}"
...
(dmesg | tail -n 200) > "$SNAP_DIR/dmesg_tail.txt" 2>/dev/null || true
