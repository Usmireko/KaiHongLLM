...
join_pids_space() {
  alive_pids | while read -r pid; do
    [ -n "$pid" ] || continue
    printf '%s ' "$pid"
  done
}
...
get_ncpu() {
  ...
  n="$(ls -d /sys/devices/system/cpu/cpu[0-9]* 2>/dev/null | wc -l 2>/dev/null)"
...
start_workers() {
  ...
  if alive_pids >/dev/null 2>&1; then
    echo "already_running pids=$(join_pids_space)"
    return 0
  fi
  ...
  echo "started workers=$workers pids_count=$(wc -l < "$PIDFILE" 2>/dev/null | sed 's/[^0-9]//g')"
  return 0
}
...
case "$cmd" in
  ...
  status)
    if alive_pids >/dev/null 2>&1; then
      cnt="$(alive_pids | wc -l 2>/dev/null | sed 's/[^0-9]//g')"
