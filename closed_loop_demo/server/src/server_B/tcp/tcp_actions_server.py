...
def handle_conn(conn: socket.socket, addr, out_root: Path) -> None:
    try:
        ...
        conclusion = ""
        conclusion_path = device_dir / "latest_conclusion.txt"
        if conclusion_path.exists():
            try:
                conclusion = conclusion_path.read_text(encoding="utf-8", errors="ignore").strip()
            except Exception:
                conclusion = ""

        if not actions_path.exists():
            conclusion_header = (f"CONCLUSION={conclusion}\n" if conclusion else "")
            conn.sendall(f"RUN={run_id}\n{conclusion_header}LEN=0\n\n".encode("utf-8"))
            return

        body = actions_path.read_bytes()
        conclusion_header = (f"CONCLUSION={conclusion}\n" if conclusion else "")
        header = f"RUN={run_id}\n{conclusion_header}LEN={len(body)}\n\n".encode("utf-8")
        conn.sendall(header)
        if body:
            conn.sendall(body)
    except Exception:
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--port", type=int, default=env_int("QWEN3_ACTIONS_PORT", 28081))
